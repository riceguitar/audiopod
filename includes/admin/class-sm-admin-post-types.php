...
class SM_Admin_Post_Types {
	...
	public function post_updated_messages( $messages ) {
		...
		$messages['wpfc_sermon'] = array(
			...
			1  => wp_sprintf( esc_html__( 'Sermon updated. %s', 'audiopod-wp' ), '<a href="' . esc_url( get_permalink( $post_ID ) ) . '">' . esc_html__( 'View sermon', 'audiopod-wp' ) . '</a>' ),
			2  => esc_html__( 'Custom field updated.', 'audiopod-wp' ),
			3  => esc_html__( 'Custom field deleted.', 'audiopod-wp' ),
			4  => esc_html__( 'Sermon updated.', 'audiopod-wp' ),
			// translators: %s: Date and time of the revision.
			5  => isset( $_GET['revision'] ) ? wp_sprintf( esc_html__( 'Sermon restored to revision from %s', 'audiopod-wp' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			// translators: %s: The URL to the sermon.
			6  => wp_sprintf( esc_html__( 'Sermon published. %s', 'audiopod-wp' ), '<a href="' . esc_url( get_permalink( $post_ID ) ) . '">' . esc_html__( 'View sermon', 'audiopod-wp' ) . '</a>' ),
			7  => esc_html__( 'Sermon saved.', 'audiopod-wp' ),
			// translators: %s: The URL to the sermon.
			8  => wp_sprintf( esc_html__( 'Sermon submitted. %s', 'audiopod-wp' ), '<a target="_blank" href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) . '">' . esc_html__( 'Preview sermon', 'audiopod-wp' ) . '</a>' ),
			// translators: %1$s: The date and time. %2$s: The preview sermon URL.
			9  => wp_sprintf( esc_html__( 'Sermon scheduled for: %1$s. %2$s', 'audiopod-wp' ),
				// translators: %1$s: Date. %2$s: Time.
				'<strong>' . wp_sprintf( esc_html__( '%1$s at %2$s', 'audiopod-wp' ), get_post_time( get_option( 'date_format' ), false, null, true ), get_post_time( get_option( 'time_format' ), false, null, true ) ) . '</strong>',
				// translators: %s: The preview sermon URL.
				'<a target="_blank" href="' . esc_url( get_permalink( $post_ID ) ) . '">' . esc_html__( 'Preview sermon', 'audiopod-wp' ) . '</a>'
			),
			// translators: %s The URL to the sermon.
			10 => wp_sprintf( esc_html__( 'Sermon draft updated. %s', 'audiopod-wp' ), '<a target="_blank" href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) . '">' . esc_html__( 'View sermon', 'audiopod-wp' ) . '</a>' ),
		);
	...
	public function sermon_columns( $existing_columns ) {
		...
		$columns['title']    = __( 'Sermon Title', 'audiopod-wp' );
		$columns['preacher'] = sm_get_taxonomy_field( 'wpfc_preacher', 'singular_name' );
		$columns['series']   = __( 'Sermon Series', 'audiopod-wp' );
		$columns['topics']   = __( 'Topics', 'audiopod-wp' );
		$columns['views']    = __( 'Views', 'audiopod-wp' );
		$columns['comments'] = $existing_columns['comments'];
		$columns['preached'] = __( 'Preached', 'audiopod-wp' );
	...
	public function enter_title_here( $text, $post ) {
		if ( 'wpfc_sermon' === $post->post_type ) {
			$text = __( 'Sermon title', 'audiopod-wp' );
		}
	...
	public function sermon_filters() {
		...
		$output .= '<option value="">' . wp_sprintf( __( 'Filter by %s', 'audiopod-wp' ), sm_get_taxonomy_field( 'wpfc_service_type', 'singular_name' ) ) . '</option>';
